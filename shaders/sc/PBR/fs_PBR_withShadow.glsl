$input v_wpos, v_tangent, v_bitangent, v_normal, v_texcoord0, v_lpos
...
void main()
{
	...
	// view
	vec3 V = normalize(u_eyePos - v_wpos).xyz;
	...
	float ka = u_k.z;
  	...
  	vec3 Lo = (ks * specular + kd * diffuse) * radiance * NdotL;
	// Shadow
	float visibility = hardShadow(s_shadowmap,v_lpos);
	// FragColor
  	vec3 color = Lo * visibility + ka * ambient * ao;
